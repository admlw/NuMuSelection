...
>>>>>>> ebeac6667386e5f53caae6874fa556a614605017
...
void ScaleHistograms(std::vector< std::vector<hists_1d*> > hists){
    for (int i_st = 0; i_st < hists.size(); i_st++){
        for (int i_pl = 0; i_pl < hists.at(i_st).size(); i_pl++){
            ...
            thisHistSet->h_mccosmic->Scale(simscaling);
            thisHistSet->h_mcmixed->Scale(simscaling);
            thisHistSet->h_mcoofv->Scale(simscaling);
            thisHistSet->h_mcnc->Scale(simscaling);
            thisHistSet->h_mcnuenuebar->Scale(simscaling);
            thisHistSet->h_mcnumubar->Scale(simscaling);
            thisHistSet->h_mcnumuccother->Scale(simscaling);
            thisHistSet->h_mcnumucc0pinp->Scale(simscaling);
            thisHistSet->h_offbeam->Scale(offbeamscaling);
        }
    }
}
