void make_data_trackfrac_ek_plot(){

    std::string varToPlot("track_range_energy_passumption");
    int nbins = 9;
    float bins[] = {0, 0.001, 0.05, 0.1, 0.15, 0.2, 0.3, 0.5, 0.8, 0.801};
    ...
    // deal with onbeam
    TH1D* num_onbeam   = new TH1D("num_onbeam",   ";Proton candidate E_{k};Track-like fraction",nbins, bins);
    TH1D* denom_onbeam = new TH1D("denom_onbeam", ";Proton candidate E_{k};Track-like fraction",nbins, bins);
    ...
    // deal with cv
    TH1D* num_cv = new TH1D("num_cv", ";Proton candidate E_{k};Track-like fraction",nbins,bins );
    TH1D* denom_cv = new TH1D("denom_cv", ";Proton candidate E_{k};Track-like fraction",nbins,bins );
    ...
    num_cv->SetBinContent(0, 0);
    num_cv->SetBinError(0, 0);
